...
from employees.serializers import EmployeeSerializer, ListEmployeeSerializer
...
class CreateEmployeeViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        idd = serializer.validated_data.get("fullname")  # type: ignore
        path = f"media/{idd}"
...
class DeleteEmployeeViewSet(DestroyAPIView):
    ...
    def get_object(self):
        ...
        obj = queryset.filter(pk=employee_id).first()
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if "media" in os.listdir(os.getcwd()):
            try:
                user_id = self.queryset.get(pk=pk).fullname
...
class UpdateEmployeeAPIView(UpdateAPIView, DestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer.data[
            "main_image"
        ] = f"{serializer.validated_data.get('fullname')}/main.jpg"
        return Response(serializer.data, status=status.HTTP_201_CREATED)
